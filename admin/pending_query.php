...
if(isset($_GET['id'])){
    ...
    header("Location: pending_query.php"); // Refresh page to show updated status
    exit;
}
...
<head>
<meta charset="UTF-8">
<title>Pending Queries - Admin</title>
...
<div class="container">
    ...
    <?php
    // Fetch all pending queries (status NULL or 2)
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        while($row = mysqli_fetch_assoc($result)){
            $status = '<a href="pending_query.php?id='.$row['query_id'].'">Pending</a>';
            echo '<tr>
                    <td>'.$count++.'</td>
        ...
        echo '<div class="alert alert-info text-center">No pending queries found.</div>';
    }

    // Pagination
    $sql_total = "SELECT COUNT(*) AS total FROM contact_query WHERE query_status IS NULL OR query_status=2";
    ...
    if($total_pages > 1){
        ...
        for($i=1; $i<=$total_pages; $i++){
            ...
            echo '<li '.$active.'><a href="pending_query.php?page='.$i.'">'.$i.'</a></li>';
        }
        if($page < $total_pages){
